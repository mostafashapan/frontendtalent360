...
const BasicInfo: React.FC<BasicInfoProps> = ({ userId }) => {
  ...
  const handleSaveChanges = async () => {
    if (formValues) {
      // Safely omit __typename by using `as any` to avoid type issues
      const updateUserInput: UpdateUserInput = { ...formValues };
  
      // If __typename exists, delete it before sending it to the mutation
      if ('__typename' in updateUserInput) {
        delete updateUserInput.__typename;
      }
...
const handleInputChange = (field: keyof UpdateUserInput, value: string) => {
  if (formValues) {
    setFormValues((prevValues) => {
      if (!prevValues) return { id: 0 } as UpdateUserInput; // fallback if prevValues is null

      // Create a new object to avoid direct mutation
      const updatedValues: UpdateUserInput = { ...prevValues };

      // Handle specific fields with custom parsing or processing
      switch (field) {
        case 'nationalities':
          // Handle `nationalities` as an array of strings
          updatedValues[field] = JSON.parse(`[${value.split(',').map(item => `"${item.trim()}"`).join(',')}]`);
          break;
        case 'dependants':
          // Handle `dependants` as a number (parse as integer)
          updatedValues[field] = parseInt(value, 10) || 0;
          break;
        case 'nationalIdNumber':
        case 'passportNumber':
          // Optionally sanitize `nationalIdNumber` or `passportNumber` if needed (remove non-numeric characters)
          updatedValues[field] = value.replace(/[^0-9]/g, '');
          break;
        case 'nationalIdExpiryDate':
        case 'passportIssueDate':
        case 'passportExpiryDate':
          // Handle date fields (ensure it's a valid date)
          updatedValues[field] = value;
          break;
        default:
          // For other fields, just assign the value directly
          updatedValues[field] = value as never;
      }
